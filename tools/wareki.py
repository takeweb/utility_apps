...
# 和暦を西暦に変換する関数
def convert_wareki_2_seireki(
    start_seireki_year: int, wareki_year_str: str, month: int, day: int
):
...
def display_streamlit_app():
    ...
    with st.expander("**西暦から和暦への変換**", expanded=True):
        ...
        # 結果表示
        if submitted1:
            wareki_result = convert_seireki_2_wareki(
                supabase_client, int(year), int(month), int(day)
            )
    ...
    with st.expander("**和暦から西暦への変換**", expanded=True):
        ...
        with cols[0]:
            selected_gengo = st.selectbox(
                "元号を選択",
                gengo_options,
                index=default_index_gengo,
                key="gengo_select_2",
            )
        ...
        if era_info:
            wareki_year_options, default_index_wareki, start_date_obj = (
                create_wareki_year_options(era_info)
            )
        ...
        if submitted2:
            if start_date_obj:
                ...
                if selected_wareki_str == "---":
                    ...
                    result = convert_wareki_2_seireki(
                        start_date_obj.year, selected_wareki_str, int(month2), int(day2)
                    )
                    if "正しい日付" in result:
                        st.error(result)
                    else:
