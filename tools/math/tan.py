...
def gen_tangent_chart(): # 引数から num_points を削除
    ...
    with st.expander("**タンジェント波 (tan) グラフ**", expanded=True):
        ...
        # プロットする点の数のスライダーを復活
        num_points = st.slider('Tangent: プロットする点の数', 100, 1000, 500, key="num_points_tan")
        ...
        fig, ax = plt.subplots()
        # 連続した線で描画する元の形式に戻します
        ax.plot(x, y_tan, label='tan(x)', color='green') # 'o' から変更, 色を 'green' に戻す
...
def gen_arctangent_chart(): # 引数から num_points を削除
    ...
    with st.expander("**アークタンジェント波 (arctan) グラフ**", expanded=True):
        ...
        # プロットする点の数のスライダーを追加
        num_points = st.slider('Arctangent: プロットする点の数', 100, 1000, 500, key="num_points_arctan")
