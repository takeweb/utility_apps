...
def select_single_range_or_custom(label, options):
    with st.expander(f"**{label}の選択**", expanded=True):
...
# --- 入力フィールド ---
with st.expander("**スケジュールを設定**", expanded=True):
    # Cron形式の切り替え
    cron_type = st.radio("**Cron形式を選択**", options=["Unix", "Spring"], index=0, horizontal=True)
