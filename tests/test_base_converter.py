...
class TestBaseConverter(unittest.TestCase):
    ...
    def test_mode1_signed_conversion(self):
        # 負の数 (-45) -> 0xFFD3
        dec, hex_out, bin_out = convert_bin_to_dec_hex("1111111111010011")
        self.assertEqual(dec, -45)
        self.assertEqual(hex_out, "0xFFD3")
        self.assertEqual(bin_out, "0b1111111111010011")
        # 短い入力 (15ビット) -> 32745 (正)
        dec, hex_out, bin_out = convert_bin_to_dec_hex("111111111101001")
        self.assertEqual(dec, 32745)
    ...
    def test_mode3_signed_conversion(self):
        # 負の数 (0xFFD3) -> -45
        bin_out, dec = convert_hex_to_bin_dec("FFD3")
        self.assertEqual(dec, -45)
        self.assertEqual(bin_out, "0b1111111111010011")
        # 正の数 (0x1234) -> 4660
        bin_out, dec = convert_hex_to_bin_dec("1234")
        self.assertEqual(dec, 4660)
        self.assertEqual(bin_out, "0b0001001000110100")

    ## モード 4: 16進 固定小数点 (8.8) → 10進
    def test_mode4_q88_to_dec(self):
        # 正の値 (0x1080) -> 16.5
        dec, int_p, frac_int, frac = convert_q88_to_dec("1080")
        self.assertAlmostEqual(dec, 16.5) # 小数点を含むため assertAlmostEqual
        self.assertEqual(int_p, 16)
        self.assertEqual(frac_int, 128)
        # 負の値 (0xBF00) -> -65.0
        dec, int_p, frac_int, frac = convert_q88_to_dec("BF00")
        self.assertEqual(int_p, -65)
        self.assertAlmostEqual(dec, -65.0)
        # 最大正 (0x7FFF) -> 127.99609375
        dec, _, _, _ = convert_q88_to_dec("7FFF")
        self.assertAlmostEqual(dec, 127 + 255/256)
